...
def bayes(ngram, essay, classifier):
    '''0 = unigram
       ...
       2 = trigram
    '''
    temp_featuresets = [(document_features(t), "young" if int(class_dic[classifier]) <= 30 else "old") for (t, class_dic) in ngram_tuple[ngram][essay] if (t and class_dic[classifier] != False and type(class_dic[classifier]) != float)]
